...
async function loadDataFromFile() {
    try {
        ...
        appendLog('Error loading file: ' + (err?.message || err));
        return [];
    }
}
...
function buildMLPModel(inputDim, layerSizes, dropout, l2) {
    const model = tf.sequential();
    let first = true;
...
function compileModel(model, learningRate, posWeight = null) {
    ...
    const loss = (posWeight && isFinite(posWeight) && posWeight > 0)
        ...
        : 'binaryCrossentropy';
    model.compile({ optimizer: opt, loss, metrics: ['binaryAccuracy'] });
}
...
async function runEDA(rows) {
    ...
    for (const c of CATEGORICAL_FEATURES) {
        ...
        const bars = Object.keys(agg).map(k => ({ index: k, value: agg[k].pos / (agg[k].cnt || 1) }));
        await tfvis.render.barchart({ name: `EDA: Churn rate by ${c}`, tab }, bars, { xLabel: c, yLabel: 'Churn rate', height: 280 });
    }
...
async function renderDecileLift(name, probs, labels, tab = 'Evaluation') {
    ...
    await tfvis.render.barchart({ name: `${name}: Decile churn rate`, tab }, ratesBars, { xLabel: 'Decile (High->Low risk)', yLabel: 'Churn rate', height: 300 });
...
async function dependencePlots(testRows, probs) {
    ...
    await tfvis.render.linechart({ name: 'Dependence: tenure', tab }, { values: binsTenure.map(b => ({ x: b.mid, y: b.avgPred })), series: ['avgPred'] }, { xLabel: 'tenure', yLabel: 'avg predicted churn', height: 280 });

    const binsMC = binDependence(testRows, probs, 'MonthlyCharges', 10);
    await tfvis.render.linechart({ name: 'Dependence: MonthlyCharges', tab }, { values: binsMC.map(b => ({ x: b.mid, y: b.avgPred })), series: ['avgPred'] }, { xLabel: 'MonthlyCharges', yLabel: 'avg predicted churn', height: 280 });

    const cats = categoryDependence(testRows, probs, 'Contract');
    await tfvis.render.barchart({ name: 'Dependence: Contract', tab }, cats, { xLabel: 'Contract', yLabel: 'avg predicted churn', height: 280 });

    const inet = categoryDependence(testRows, probs, 'InternetService');
    await tfvis.render.barchart({ name: 'Dependence: InternetService', tab }, inet, { xLabel: 'InternetService', yLabel: 'avg predicted churn', height: 280 });
}
...
async function main() {
    ...
    document.getElementById('btnEDA').addEventListener('click', async () => {
        if (!STATE.rawRows.length) { appendLog('Please upload CSV and click Load Data first.'); return; }
    ...
    document.getElementById('btnPreprocess').addEventListener('click', async () => {
        ...
        const tTrain = transformRows(trainRows, STATE.maps);
        const tVal = transformRows(valRows, STATE.maps);
        const tTest = transformRows(testRows, STATE.maps);
    ...
    document.getElementById('btnTrainNN').addEventListener('click', async () => {
        ...
        const inputDim = STATE.transformed.train.nCols;
        const model = buildMLPModel(inputDim, layerSizes.length ? layerSizes : [128, 64], dropout, l2);
        compileModel(model, lr, STATE.posWeight);
    ...
    document.getElementById('btnFormatJson').addEventListener('click', () => {
        ...
        try {
            if (!area.value.trim()) { out.textContent = 'Nothing to format.'; return; }
