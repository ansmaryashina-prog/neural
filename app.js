/* Telco Churn EDA + NN + Business Simulator - Client-only
Uses: TensorFlow.js, tfjs-vis, PapaParse
Author: In-browser assistant
*/

const DATA_URL = 'https://raw.githubusercontent.com/IBM/employee-attrition-aif360/master/data/WA_Fn-UseC_-Telco-Customer-Churn.csv';
...
const STATE = {
    ...
    rawRows: [],
    cleanedRows: [],
    split: null, // {trainIdxs,valIdxs,testIdxs, stats}
    maps: null, // preprocessing maps
    transformed: null, // {train:{X,y,meta}, val:{...}, test:{...}}
...
async function loadData() {
    ...
    STATE.rawRows = rows;
    appendLog(`Loaded ${rows.length} rows, ${parsed.meta.fields.length} columns.`);
    return rows;
}
...
function imputeAndClean(rows) {
    ...
    for (const r of cleaned) {
        ...
        // TotalCharges: sometimes blank -> NaN
    ...
    for (const r of cleaned) {
        ...
        r.TotalCharges = tc;
        // monthly charges may be null, coerce to mean if needed
...
function buildPreprocessingMaps(trainRows) {
    ...
    for (const c of CATEGORICAL_FEATURES) {
        ...
        const cats = Array.from(set.values());
        categoryMaps[c] = cats;
    }
    const numStats = {};
...
function buildLogisticModel(inputDim, l2) {
    ...
    model.add(tf.layers.dense({ units: 1, activation: 'sigmoid', kernelRegularizer: tf.regularizers.l2({l2}), inputShape: [inputDim] }));
    return model;
}
...
function buildMLPModel(inputDim, layerSizes, dropout, l2, learningRate) {
    ...
    for (let i = 0; i < layerSizes.length; i++) {
        ...
        model.add(tf.layers.dense({
            units, activation: 'relu', inputShape: first ? [inputDim] : undefined,
...
function prCurve(probs, labels) {
    ...
    for (let i = 0; i < pairs.length; i++) {
        ...
        if (prob !== lastProb) {
            ...
            precision.push(prec); recall.push(rec);
            lastProb = prob;
        }
        if (y === 1) tp++; else fp++;
    }
...
async function runEDA(rows) {
    ...
    const tab = 'EDA';
    // Churn distribution
    const churnRate = rows.reduce((a,r)=>a + (r._y===1?1:0), 0) / rows.length;
    await tfvis.render.barchart({name:'EDA: Churn Distribution', tab}, {
        values: [{x: 'No', y: rows.length - rows.reduce((a,r)=>a+(r._y===1?1:0), 0)}, {x:'Yes', y: rows.reduce((a,r)=>a+(r._y===1?1:0),0)}],
        series: ['count']
    ...
    for (const c of CATEGORICAL_FEATURES) {
        ...
        for (const r of rows) {
            ...
            agg[v].cnt++; agg[v].pos += (r._y===1?1:0);
        }
    ...
    appendLog(`EDA complete. Churn rate overall: ${(100*churnRate).toFixed(1)}%.`);
}

// Business value computations
function expectedValuePerCustomer(p, monthlyCharge, params) {
    const offerCost = params.offerCost;
    const saveRate = params.saveRate;
    const months = params.retentionMonths;
    const margin = params.margin;
    const m = isFinite(monthlyCharge) ? monthlyCharge : 0;
    return (p * saveRate * margin * m * months) - offerCost;
}

function businessProfitThresholdSweep(probs, labels, monthly, params) {
    const results = [];
    for (let t=0.1;t<=0.9;t+=0.01) {
        let targeted = 0, profit = 0, tp=0, fp=0, pos=0;
        for (let i=0;i<probs.length;i++) {
            pos += labels[i];
            if (probs[i] >= t) {
                targeted++;
...
function segmentMetrics(rows, probs, labels) {
    ...
    function segmentAUC(subProbs, subLabels) {
        ...
        const auc = aucFromCurve(pairs.map(p=>p[0]), pairs.map(p=>p[1]));
        return auc;
    }
    ...
    for (const [key, values] of Object.entries(segments)) {
        for (const v of values) {
            ...
            results.push({ segment: `${key}=${v}`, size: subProbs.length, auc: segmentAUC(subProbs, subLabels), ece: segmentECE(subProbs, subLabels) });
        }
    }
    return results.sort((a,b)=>a.segment.localeCompare(b.segment));
}

// Rendering helpers using tfjs-vis
...
function getHyperparams() {
    const layersStr = document.getElementById('hpLayers').value.trim();
    const layerSizes = layersStr ? layersStr.split(',').map(s=>parseInt(s.trim(),10)).filter(n=>!isNaN(n) && n>0) : [];
...
async function main() {
    document.getElementById('btnLoad').addEventListener('click', async () => {
        await loadData();
        appendLog('Done. Next: Run EDA or Preprocess & Split.');
    });

    document.getElementById('btnEDA').addEventListener('click', async () => {
        if (!STATE.rawRows.length) await loadData();
    ...
    document.getElementById('btnTrainBaseline').addEventListener('click', async () => {
        if (!STATE.transformed) { appendLog('Please Preprocess & Split first.'); return; }
    ...
    document.getElementById('btnEvaluate').addEventListener('click', async () => {
        ...
        // Explainability (permutation importance) - subsample val
        ...
        const top15 = pi.importance.slice(0,15).map(x=>({x: x.feature, y: x.delta}));
        await tfvis.render.barchart({name:`Permutation Importance (PR AUC drop)`, tab:'Explainability'}, {values: top15, series:['delta PRAUC']}, { xLabel:'Feature', yLabel:'ΔPR AUC', height: 300 });
        ...
        await dependencePlots(testRows, probs);
    });
    ...
    document.getElementById('btnSetF1').addEventListener('click', () => {
        if (STATE.thresholds.f1 == null) { appendLog('Train NN to compute F1-optimal threshold.'); return; }
    ...
    document.getElementById('btnSaveModel').addEventListener('click', async () => {
        if (!STATE.models.mlp) { appendLog('No NN model to save.'); return; }
    ...
    document.getElementById('btnDownloadPrep').addEventListener('click', async () => {
        if (!STATE.maps) { appendLog('No preprocessing maps to download.'); return; }
    ...
    document.getElementById('btnScoreRow').addEventListener('click', async () => {
        ...
        if (!STATE.models.mlp || !STATE.maps) { appendLog('Need trained NN and preprocessing to score.'); return; }
        try {
            const row = JSON.parse(txt);
            // Normalize keys to match CSV
    ...
    appendLog('Ready. Start with Load Data.');
}
...
async function dependencePlots(testRows, probs) {
    ...
    await tfvis.render.linechart({name:'Dependence: tenure', tab}, {values: binsTenure.map((b,i)=>({x: b.mid, y: b.avgPred})), series:['avgPred']}, { xLabel:'tenure', yLabel:'avg predicted churn', height: 280 });
    const binsMC = binDependence(testRows, probs, 'MonthlyCharges', 10);
    await tfvis.render.linechart({name:'Dependence: MonthlyCharges', tab}, {values: binsMC.map((b,i)=>({x: b.mid, y: b.avgPred})), series:['avgPred']}, { xLabel:'MonthlyCharges', yLabel:'avg predicted churn', height: 280 });
    // categorical bars avg predicted churn
    const cats = categoryDependence(testRows, probs, 'Contract');
    await tfvis.render.barchart({name:'Dependence: Contract', tab}, {values: cats, series:['avgPred']}, { xLabel:'Contract', yLabel:'avg predicted churn', height: 280 });
    const inet = categoryDependence(testRows, probs, 'InternetService');
    await tfvis.render.barchart({name:'Dependence: InternetService', tab}, {values: inet, series:['avgPred']}, { xLabel:'InternetService', yLabel:'avg predicted churn', height: 280 });
}
...
function binDependence(rows, probs, col, nBins) {
    ...
    for (let b=0;b<nBins;b++) {
        ...
        const slice = pairs.slice(start, end);
        const avgP = slice.reduce((a,s)=>a+s.p,0)/(slice.length||1);
        const mid = (slice[0]?.x ?? 0 + slice[slice.length-1]?.x ?? 0)/2;
...
function categoryDependence(rows, probs, col) {
    ...
    return Object.keys(agg).map(k => ({x: k, y: agg[k].sum/(agg[k].cnt||1)}));
}
